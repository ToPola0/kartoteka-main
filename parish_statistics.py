...
class Statistics:
    ...
    def add_person(self, person):
        ...
        # Wiek
        age = person.get("wiek")
        is_unknown = person.get("is_unknown_date", False)
        if age is not None:
            # Dodaj wiek do statystyk tylko jeśli to nie jest data 99/99/9999
            # (mediana powinna być obliczana tylko z rzeczywistych dat)
            if not is_unknown:
                self.ages.append(age)  # Zapisz wiek do listy
            if age < 18:
    ...
    def get_current_median_age(self):
        """Zwraca aktualną medianę wieku lub 50 jeśli brak danych."""
        if not self.ages:
            return 50  # Domyślny wiek gdy brak statystyk
